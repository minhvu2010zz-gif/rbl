-- MV Hub v5.0 | FULL COMMANDS + NIGHT VISION
-- Siêu Nhảy = nhảy cao | Bay = nhấn Space | Xuyên Tường = xuyên rào cản
-- ESP Quái = định vị quái | ESP Người = định vị người chơi
-- Nhìn Đêm = bật sáng trong tối
...
-- // ============ TOGGLES ============
local Toggles = {
    FixLag = false,        -- Tắt hiệu ứng để giảm lag
    SuperJump = false,     -- NHẢY CAO
    Fly = false,           -- BAY (nhấn Space để bay lên)
    Noclip = false,        -- XUYÊN TƯỜNG (xuyên rào cản)
    Ghost = false,         -- TÀNG HÌNH
    NightVision = false,   -- 🌙 NHÌN ĐÊM (bật sáng trong tối)
    ESPPlayers = false,    -- ĐỊNH VỊ NGƯỜI CHƠI
    ESPMobs = false,       -- ĐỊNH VỊ QUÁI
    ESPFruits = false,     -- ĐỊNH VỊ TRÁI CÂY
    AutoFarm = false       -- TỰ ĐỘNG FARM
}
...
-- // ============ NIGHT VISION (BẬT SÁNG TRONG TỐI) ============
local function RunNightVision()
    spawn(function()
        while wait(0.5) do
            if Toggles.NightVision then
                -- BẬT SÁNG TRONG TỐI
                Lighting.Brightness = 5          -- Tăng độ sáng tối đa
                Lighting.ClockTime = 12          -- Đặt giờ ban ngày
                Lighting.FogEnd = 99999          -- Xóa sương mù
                Lighting.GlobalShadows = false   -- Tắt bóng tối
                Lighting.Ambient = Color3.fromRGB(255, 255, 255)  -- Ánh sáng trắng
                -- Tắt hiệu ứng gây tối
                for _, v in pairs(Lighting:GetDescendants()) do
...
local function CreateUI()
    ...
    AddToggle(MainPanel, "SuperJump", "🦘 Siêu Nhảy (Nhảy Cao)")
    AddToggle(MainPanel, "Fly", "✈️ Bay (Nhấn Space)")
    AddToggle(MainPanel, "Noclip", "👻 Xuyên Tường")
    AddToggle(MainPanel, "Ghost", "👻 Tàng Hình")
    AddToggle(MainPanel, "NightVision", "🌙 Nhìn Đêm (Bật Sáng Trong Tối)")
    ...
    AddToggle(ESPPanel, "ESPPlayers", "👤 ESP Người Chơi (Định Vị Người)")
    AddToggle(ESPPanel, "ESPMobs", "👾 ESP Quái (Định Vị Quái)")
...
print("⚡ MV HACK v5.0 - FULL COMMANDS LOADED")
print("✅ Siêu Nhảy = Nhảy Cao")
print("✅ Bay = Nhấn Space để bay lên")
print("✅ Xuyên Tường = Xuyên rào cản")
print("✅ ESP Quái = Định vị quái")
print("✅ ESP Người = Định vị người chơi")
print("✅ Nhìn Đêm = Bật sáng trong tối")
print("💀 Boss man, fuck yeah! All commands working!")
