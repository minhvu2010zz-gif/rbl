-- MV Hub Reborn v5.0 | MOBILE FIXED v3
-- Sửa lỗi scroll, ngôn ngữ, tất cả lệnh
...
-- UI References (để refresh ngôn ngữ)
local UIReferences = {
    TitleLabel = nil,
    ToggleButtons = {},
    SliderLabels = {},
...
-- // ============ REFRESH UI (FIX LANGUAGES) ============
local function RefreshUI()
    if UIReferences.TitleLabel then
        UIReferences.TitleLabel.Text = Lang.title
    end
    
    for _, btn in pairs(UIReferences.ToggleButtons) do
    ...
    if UIReferences.StatusLabel then
        if Toggles.AutoFarm and SelectedMob then
            UIReferences.StatusLabel.Text = Lang.status_farming
        else
            UIReferences.StatusLabel.Text = Lang.status_standing
        end
    end
    
    if UIReferences.MobCountLabel then
        UIReferences.MobCountLabel.Text = Lang.mobs_found .. #FarmTargets
    end
    
    if UIReferences.SelectedText then
        UIReferences.SelectedText.Text = "📌 " .. SelectedMapName
    end
end
...
local function CreateUI()
    ...
    UIReferences.TitleLabel = TitleLabel
    ...
    LangBtn.MouseButton1Click:Connect(function()
        ...
        RefreshUI()  -- Cập nhật tất cả text
    end)
    ...
    local function CreateContentPanel(tabName)
        ...
        panel.AutomaticCanvasSize = Enum.AutomaticSize.XY  -- FIX: Tự động co giãn
    ...
    local function AddMainToggle(panel, labelKey, toggleKey)
        ...
        table.insert(UIReferences.ToggleButtons, btn)
    ...
    local function AddSlider(panel, labelKey, varRef, minVal, maxVal, step)
        ...
        label.TextScaled = true
        label.Font = Enum.Font.GothamBold
        table.insert(UIReferences.SliderLabels, label)
    ...
    table.insert(UIReferences.SliderLabels, mobLabel)
    ...
    table.insert(UIReferences.SliderLabels, weaponLabel)
    ...
    UIReferences.StatusLabel = statusLabel
    ...
    UIReferences.MobCountLabel = mobCountLabel
    ...
    table.insert(UIReferences.SliderLabels, fovLabel)
    ...
    table.insert(UIReferences.SliderLabels, colorLabel)
    ...
    local function BuildMapUI()
        ...
        UIReferences.SelectedText = selectedText
        ...
        for i, mapData in ipairs(DetectedMapPoints) do
            ...
            table.insert(UIReferences.MapButtons, mapBtn)
...
print("⚡ MV HUB v5.0 MOBILE FIXED v3 LOADED")
print("📱 Touch: Tap MV icon | Swipe to toggle | Scroll menu")
print("🛸 Fly button on right side")
print("🌐 Language button works now!")
print("💀 Boss man, fuck yeah! All fixed!")
