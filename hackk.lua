-- MV Hub | Axiom Build v4.5
-- FULL FIXED - No Empty Menu
...
ZoomInBtn.MouseButton1Click:Connect(function()
    ...
    MainFrame.Size = UDim2.new(0, 620 * scale, 0, 520 * scale)
    MainFrame.Position = UDim2.new(0.5, -310 * scale, 0.5, -260 * scale)
end)
...
-- // ===== TAB MAIN =====
AddLabel(tabMain, "🎮 MV HUB v4.5 - Full Fixed")
...
local toggleAutoFarm = AddToggle(tabFeatures, "🤖 Auto Farm", false, function(state)
...
AddInfoCard(tabAdmin, {
    { label = "👤 Tên", key = "name", value = "MV Boss" },
    { label = "📅 Ngày sinh", key = "birthday", value = "01/01/2000" },
    { label = "📝 Bio", key = "bio", value = "⚡ MV HUB Developer" },
    { label = "🕐 Hoạt động", key = "active", value = "24/7" },
    { label = "💻 Version", key = "version", value = "v4.5 - Full Fixed" },
})

AddButton(tabAdmin, "📢 Thông báo", function()
    print("📢 MV HUB v4.5 - Đã sẵn sàng!")
end)
...
langKO.MouseButton1Click:Connect(function()
    currentLang = "ko"
    langKO.BackgroundColor3 = Theme.Accent
    langVI.BackgroundColor3 = Theme.TabInactive
    langEN.BackgroundColor3 = Theme.TabInactive
    print("🌐 한국어로 전환되었습니다")
end)
...
local zoomSlider = AddSlider(tabSettings, "🔍 Thu phóng", 0.6, 1.5, 1, function(val)
    scale = val
    MainFrame.Size = UDim2.new(0, 620 * val, 0, 520 * val)
    MainFrame.Position = UDim2.new(0.5, -310 * val, 0.5, -260 * val)
end)
    ...
    spawn(function()
        while fruitTeleportEnabled and Toggles.AutoTeleportFruit do
            ...
            if nearestFruit and nearestDist < 500 then
                ...
                print("🍎 Đã dịch chuyển đến trái cây!")
                wait(1)
            end
            wait(0.5)
        end
...
print("⚡ MV HUB v4.5 - FULL FIXED - LOADED SUCCESSFULLY!")
