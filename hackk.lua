-- MV X SHINN DEV | Axiom Build v6.0
...
-- Title Bar (đặt trong Frame riêng)
...
TitleLabel.Text = "⚡ MV X SHINN DEV v6.0"
...
local function AddSliderWithButtons(tab, text, minVal, maxVal, default, callback)
    ...
    minusBtn.BackgroundColor3 = Theme.Danger
    minusBtn.Text = "-"
    ...
    plusBtn.BackgroundColor3 = Theme.Success
    plusBtn.Text = "+"
...
-- // ===== TAB MAIN =====
AddLabel(tabMain, "🎮 MV X SHINN DEV v6.0 - Full Features")
...
-- 4. Noclip (Fixed - Không bị lỗi xuống đất)
function EnableNoclip()
    ...
    if char then
        ...
        -- Đặt lại vị trí để tránh bị rơi xuống đất
    ...
    spawn(function()
        while wait(0.5) do
            if Toggles.NightVision then
                ...
                -- Tắt tất cả hiệu ứng ánh sáng tối
...
local function ESPLoop()
    spawn(function()
        while wait(0.5) do
            ...
            -- Mob ESP - Tất cả quái (bao gồm quái thường)
            if Toggles.ESPMobs then
                for _, v in pairs(Workspace:GetDescendants()) do
                    if v:IsA("Model") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
                        local name = v.Name:lower()
                        -- Bắt tất cả các loại quái
            ...
            -- Fruit ESP - Chỉ tìm trái cây, không tìm NPC bán trái
            if Toggles.ESPFruits then
                for _, v in pairs(Workspace:GetDescendants()) do
                    if v:IsA("Model") then
                        local name = v.Name:lower()
                        -- Chỉ tìm trái cây, không tìm NPC, seller, vendor
...
print("⚡ MV X SHINN DEV v6.0 - FULL FIXED - LOADED SUCCESSFULLY!")
...
ShowNotification("⚡ MV X SHINN DEV v6.0", "Đã tải thành công! Nhấn M để mở menu")
