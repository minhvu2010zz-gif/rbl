-- MV X SHINN DEV Hub v5.2
-- FIX: Toggle Button Icon MV + Menu Render
...
local Lang = {
    vi={
        title="MV X SHINN DEV v5.2",home="Trang chủ",features="Tính năng",
        esp="ESP",map="Map",admin="Admin",settings="Cài đặt",
        fixlag="Fix Lag",superjump="Super Jump",fly="Bay (F)",
        ...
        autotpfruit="Auto TP Trái",flyspeed="Tốc độ bay",jumpforce="Lực nhảy",
    ...
    en={
        title="MV X SHINN DEV v5.2",home="Home",features="Features",
        esp="ESP",map="Map",admin="Admin",settings="Settings",
        fixlag="Fix Lag",superjump="Super Jump",fly="Fly (F)",
        noclip="Noclip",autofarm="Auto Farm",autofarmv2="Farm V2 (Dame 9999)",
        espplayer="Player ESP",espmob="Mob ESP",espfruit="Fruit ESP",
        autotpfruit="Auto TP Fruit",flyspeed="Fly Speed",jumpforce="Jump Force",
    ...
    ko={
        title="MV X SHINN DEV v5.2",home="홈",features="기능",
        esp="ESP",map="맵",admin="관리자",settings="설정",
        fixlag="렉 수정",superjump="슈퍼 점프",fly="비행 (F)",
        noclip="노클립",autofarm="자동 파밍",autofarmv2="파밍 V2 (데미지 9999)",
        espplayer="플레이어 ESP",espmob="몹 ESP",espfruit="열매 ESP",
        autotpfruit="자동 TP 열매",flyspeed="비행 속도",jumpforce="점프력",
...
local Theme = {
    ...
    MVBlue    = Color3.fromRGB(30,80,220),
    MVGold    = Color3.fromRGB(255,200,50),
}
...
local function Stroke(obj, color, thickness)
    ...
    s.Color = color or Color3.fromRGB(80,110,255)
...
-- ============================================================
-- // TOGGLE BUTTON — MV ICON (custom drawn, no image needed)
-- ============================================================
local OpenHolder = Inst("Frame", {
    Parent = SG,
    Size = UDim2.new(0, 62, 0, 62),
    Position = UDim2.new(0, 14, 0.5, -31),
    BackgroundColor3 = Theme.MVBlue,
    BorderSizePixel = 0,
    ZIndex = 50,
})
Corner(OpenHolder, 14)
Stroke(OpenHolder, Color3.fromRGB(100,160,255), 2)
...
grad.Rotation = 135
grad.Parent = OpenHolder

-- MV text label
local MVLabel = Inst("TextLabel", {
    Parent = OpenHolder,
...
spawn(function()
    while wait(1.2) do
        TweenService:Create(OpenHolder, TweenInfo.new(0.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
...
Corner(MF, 14)
Stroke(MF, Color3.fromRGB(80,110,255), 1.5)
...
-- Title MV icon inside titlebar
local TitleIconFrame = Inst("Frame", {
    Parent = TB,
...
TitleIconGrad.Rotation = 135
TitleIconGrad.Parent = TitleIconFrame
...
local function TBtn(txt, xOff, col)
    local b = Inst("TextButton", {
        Parent = TB,
        Size = UDim2.new(0, 30, 0, 30),
        Position = UDim2.new(1, xOff, 0.5, -15),
        BackgroundColor3 = col or Theme.TabOn,
        Text = txt,
        TextColor3 = Color3.new(1,1,1),
        TextScaled = true,
        Font = Enum.Font.GothamBold,
...
local CloseBtn   = TBtn("✕", -42,  Theme.Danger)
local ZoomInBtn  = TBtn("+", -80,  Theme.TabOn)
local ZoomOutBtn = TBtn("−", -116, Theme.TabOn)
...
local function ToggleMenu()
    ...
    MF.Visible = true
    if menuOpen then
        MF.Size = UDim2.new(0, BW*scale*0.8, 0, BH*scale*0.8)
        MF.BackgroundTransparency = 0.5
        TweenService:Create(MF, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, BW*scale, 0, BH*scale),
            BackgroundTransparency = 0,
        }):Play()
    else
        TweenService:Create(MF, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Size = UDim2.new(0, BW*scale*0.85, 0, BH*scale*0.85),
            BackgroundTransparency = 0.5,
        }):Play()
        task.delay(0.22, function() if not menuOpen then MF.Visible=false end end)
    end
end
...
Inst("UIListLayout", {
    Parent=Sidebar, SortOrder=Enum.SortOrder.LayoutOrder, Padding=UDim.new(0,4)
})
Pad(Sidebar, 8, 7, 8, 7)
...
local TAB_ICONS = {
    main="🏠", features="⚡", esp="👁", map="🗺", admin="👤", settings="⚙"
}

local function MakeTab(id, labelKey)
    local icon = TAB_ICONS[id] or "•"
    ...
    Inst("UIListLayout", {
        Parent=page, SortOrder=Enum.SortOrder.LayoutOrder, Padding=UDim.new(0,6)
    })
...
local function WLabel(tab, txt, color)
    return Inst("TextLabel", {
        Parent=tab.Page, Size=UDim2.new(1,0,0,22),
        BackgroundTransparency=1, Text=txt,
        TextColor3=color or Theme.Accent, Font=Enum.Font.GothamBold,
        TextSize=13, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=12,
    })
end

local function WButton(tab, txt, cb, color)
    local b = Inst("TextButton", {
        Parent=tab.Page, Size=UDim2.new(1,0,0,38),
        BackgroundColor3=color or Theme.Accent, Text=txt,
        TextColor3=Color3.new(1,1,1), Font=Enum.Font.GothamBold,
        TextSize=13, BorderSizePixel=0, ZIndex=12,
    })
    Corner(b,8)
...
local function WToggle(tab, txt, default, cb)
    ...
    local row = Inst("Frame", {
        Parent=tab.Page, Size=UDim2.new(1,0,0,40),
        BackgroundColor3=Theme.Panel, BorderSizePixel=0, ZIndex=12,
    })
    Corner(row,8)
    local lbl = Inst("TextLabel", {
        Parent=row, Size=UDim2.new(1,-62,1,0), Position=UDim2.new(0,12,0,0),
        BackgroundTransparency=1, Text=txt, TextColor3=Theme.Text,
    ...
    local knob = Inst("Frame", {
        Parent=sbg, Size=UDim2.new(0,20,0,20),
        Position=state and UDim2.new(1,-22,0.5,-10) or UDim2.new(0,2,0.5,-10),
        BackgroundColor3=Color3.new(1,1,1), BorderSizePixel=0, ZIndex=14,
    })
    Corner(knob,10)
...
local function WSlider(tab, txt, mn, mx, def, cb)
    local val = def or mn
    local c = Inst("Frame", {
        Parent=tab.Page, Size=UDim2.new(1,0,0,56),
        BackgroundColor3=Theme.Panel, BorderSizePixel=0, ZIndex=12,
    })
    Corner(c,8)
    Pad(c,8,12,8,12)
    local lbl = Inst("TextLabel", {
        Parent=c, Size=UDim2.new(0.65,0,0,20), BackgroundTransparency=1,
        Text=txt, TextColor3=Theme.TextDim, Font=Enum.Font.Gotham,
        TextSize=12, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13,
    })
    local valLbl = Inst("TextLabel", {
        Parent=c, Size=UDim2.new(0.35,0,0,20), Position=UDim2.new(0.65,0,0,0),
        BackgroundTransparency=1, Text=tostring(val), TextColor3=Theme.Accent,
        Font=Enum.Font.GothamBold, TextSize=14,
        TextXAlignment=Enum.TextXAlignment.Right, ZIndex=13,
    })
    local track = Inst("Frame", {
        Parent=c, Size=UDim2.new(1,0,0,8), Position=UDim2.new(0,0,0,30),
        BackgroundColor3=Theme.TabOff, BorderSizePixel=0, ZIndex=13,
    })
    Corner(track,4)
    local fill = Inst("Frame", {
        Parent=track, Size=UDim2.new((val-mn)/(mx-mn),0,1,0),
        BackgroundColor3=Theme.Accent, BorderSizePixel=0, ZIndex=14,
    })
    Corner(fill,4)
    local knob = Inst("Frame", {
        Parent=track, Size=UDim2.new(0,14,0,14),
        BackgroundColor3=Color3.new(1,1,1), BorderSizePixel=0, ZIndex=15,
    })
    Corner(knob,7)
...
local function WInfoCard(tab, rows)
    ...
    local card = Inst("Frame", {
        Parent=tab.Page, Size=UDim2.new(1,0,0,h),
        BackgroundColor3=Theme.Panel, BorderSizePixel=0, ZIndex=12,
    })
    Corner(card,8)
    ...
    for _,r in ipairs(rows) do
        local row=Inst("Frame",{Parent=card,Size=UDim2.new(1,0,0,24),BackgroundTransparency=1,ZIndex=13})
        Inst("TextLabel",{
            Parent=row,Size=UDim2.new(0.45,0,1,0),BackgroundTransparency=1,
            Text=r.label..":",TextColor3=Theme.TextDim,Font=Enum.Font.Gotham,
        ...
        local v=Inst("TextLabel",{
            ...
            BackgroundTransparency=1,Text=r.value or "",TextColor3=Theme.Text,
...
local function WDropdown(tab, labelTxt, options, default, cb)
    ...
    local hdr = Inst("TextButton", {
        Parent=wrap, Size=UDim2.new(1,0,0,40), BackgroundTransparency=1,
        Text=labelTxt..": "..sel.."  ▾", TextColor3=Theme.Text,
    ...
    local drop = Inst("Frame", {
        Parent=wrap, Size=UDim2.new(1,0,0,dropH),
        Position=UDim2.new(0,0,0,42),
        BackgroundColor3=Theme.Panel, BorderSizePixel=0,
        Visible=false, ZIndex=30, ClipsDescendants=false,
    })
    Corner(drop,8)
    Stroke(drop, Theme.Accent, 1)
    Inst("UIListLayout",{Parent=drop,Padding=UDim.new(0,2)})
    Pad(drop,4,6,4,6)
    for _,opt in ipairs(options) do
        local ob=Inst("TextButton",{
            Parent=drop, Size=UDim2.new(1,0,0,30),
            BackgroundColor3=(opt==sel) and Theme.Accent or Theme.TabOff,
            Text=opt, TextColor3=Color3.new(1,1,1),
            Font=Enum.Font.Gotham, TextSize=13, BorderSizePixel=0, ZIndex=31,
        })
        Corner(ob,6)
...
local tESP      = MakeTab("esp",      "esp")
local tMap      = MakeTab("map",      "map")
local tAdmin    = MakeTab("admin",    "admin")
local tSettings = MakeTab("settings", "settings")

-- ============================================================
-- // TAB: HOME
-- ============================================================
-- MV banner inside home
local bannerFrame = Inst("Frame", {
    Parent=tMain.Page, Size=UDim2.new(1,0,0,70),
    BackgroundColor3=Theme.MVBlue, BorderSizePixel=0, ZIndex=12,
})
Corner(bannerFrame,10)
local bannerGrad = Instance.new("UIGradient")
bannerGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(50,100,255)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(30,60,200)),
...
Inst("TextLabel", {
    Parent=bannerFrame, Size=UDim2.new(1,0,0.6,0), Position=UDim2.new(0,0,0.05,0),
    BackgroundTransparency=1, Text="MV X SHINN DEV",
    TextColor3=Color3.new(1,1,1), TextScaled=true,
    Font=Enum.Font.GothamBlack, ZIndex=13,
})
Inst("TextLabel", {
    Parent=bannerFrame, Size=UDim2.new(1,0,0.35,0), Position=UDim2.new(0,0,0.63,0),
    BackgroundTransparency=1, Text="v5.2  —  Full Fixed",
    TextColor3=Theme.MVGold, TextScaled=true,
...
local togFixLag    = WToggle(tFeatures, T("fixlag"),    false, function(s) Toggles.FixLag=s end)
local lagSlider    = WSlider(tFeatures, T("lagreduction"), 0, 99, 80, function(v) LagReduction=v end)
local togSuperJump = WToggle(tFeatures, T("superjump"), false, function(s) Toggles.SuperJump=s end)
...
local speedSlider = WSlider(tFeatures, T("flyspeed"), 10, 300, 50, function(v) FlySpeed=v end)
...
local togESPP = WToggle(tESP, T("espplayer"), false, function(s)
    Toggles.ESPPlayers=s; if not s then ClearESPType("player") end
end)
local togESPM = WToggle(tESP, T("espmob"), false, function(s)
    Toggles.ESPMobs=s; if not s then ClearESPType("mob") end
end)
local togESPF = WToggle(tESP, T("espfruit"), false, function(s)
    Toggles.ESPFruits=s; if not s then ClearESPType("fruit") end
end)
...
local mapScroll = Inst("ScrollingFrame", {
    Parent=tMap.Page, Size=UDim2.new(1,0,0,220),
    BackgroundColor3=Theme.Panel, BorderSizePixel=0,
    ScrollBarThickness=5, ScrollBarImageColor3=Theme.Accent,
    CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y,
    ScrollingDirection=Enum.ScrollingDirection.Y, ClipsDescendants=true, ZIndex=12,
})
Corner(mapScroll,8)
...
function BuildMapUI()
    ...
    Inst("TextLabel",{
        ...
        TextSize=11, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13,
    })
    for _,md in ipairs(DetectedMapPoints) do
        ...
        Pad(btn,0,0,0,8); Corner(btn,6)
        btn.MouseButton1Click:Connect(function()
            SelectedMapPoint=md.Position; SelectedMapName=md.Name
...
-- // TAB: ADMIN
-- ============================================================
local adminBanner = Inst("Frame", {
    Parent=tAdmin.Page, Size=UDim2.new(1,0,0,60),
    BackgroundColor3=Theme.MVBlue, BorderSizePixel=0, ZIndex=12,
})
Corner(adminBanner,10)
local ag=Instance.new("UIGradient")
ag.Color=ColorSequence.new({
    ColorSequenceKeypoint.new(0,Color3.fromRGB(50,100,255)),
    ColorSequenceKeypoint.new(1,Color3.fromRGB(80,30,180)),
})
ag.Rotation=135; ag.Parent=adminBanner
Inst("TextLabel",{
    Parent=adminBanner, Size=UDim2.new(1,0,1,0),
    BackgroundTransparency=1, Text="MV X SHINN DEV",
    TextColor3=Color3.new(1,1,1), TextScaled=true,
    Font=Enum.Font.GothamBlack, ZIndex=13,
})
...
WInfoCard(tAdmin,{
    ...
    {label="Version",key="v", value="v5.2 Full Fixed"},
    {label="Status", key="s", value="✅ Active"},
    {label="Online", key="t", value="24/7"},
})
WButton(tAdmin,"📢 Thông báo",function() print("⚡ MV X SHINN DEV v5.2!") end)
...
local function ApplyLang(lang)
    ...
    TitleLbl.Text="MV X SHINN DEV  v5.2"
    for _,t in pairs(AllTabData) do
        t.Button.Text=t.Icon.." "..T(t.LabelKey)
    end
    togFixLag.Label.Text=T("fixlag"); togSuperJump.Label.Text=T("superjump")
    togFly.Label.Text=T("fly"); togNoclip.Label.Text=T("noclip")
    togAutoFarm.Label.Text=T("autofarm"); togAutoFarmV2.Label.Text=T("autofarmv2")
    togESPP.Label.Text=T("espplayer"); togESPM.Label.Text=T("espmob")
    togESPF.Label.Text=T("espfruit"); togAutoTP.Label.Text=T("autotpfruit")
    speedSlider.Label.Text=T("flyspeed"); jumpSlider.Label.Text=T("jumpforce")
    lagSlider.Label.Text=T("lagreduction")
    selLbl.Text=T("selected")..(SelectedMapName or T("notselected"))
end
...
local zoomSld=WSlider(tSettings,"🔍 Zoom (%)",50,200,100,function(v) ApplyScale(v/100) end)
WButton(tSettings,T("save"),function() print("💾 Saved!") end)
WButton(tSettings,T("reset"),function() ApplyScale(1);zoomSld.Set(100) end, Theme.Warning)
...
UserInputService.InputBegan:Connect(function(i,gp)
    ...
    if flyEnabled then
        bodyVelocity=Inst("BodyVelocity",{MaxForce=Vector3.new(1e6,1e6,1e6),Velocity=Vector3.zero,Parent=r})
    else
        if bodyVelocity then bodyVelocity:Destroy();bodyVelocity=nil end
    end
end)
...
spawn(function()
    while wait(0.1) do
        if not Toggles.Ghost then continue end
        local c=LocalPlayer.Character
...
spawn(function()
    while wait(0.5) do
        ...
        elseif not Toggles.FixLag then
            Lighting.Brightness=NVorig.Brightness;Lighting.ClockTime=NVorig.ClockTime
            Lighting.FogEnd=NVorig.FogEnd;Lighting.GlobalShadows=NVorig.GlobalShadows
...
local function MakeESP(obj,col,txt,typ)
    ...
    local dl=Inst("TextLabel",{
        ...
        BackgroundTransparency=1,Text="0m",TextColor3=Color3.fromRGB(255,255,100),
        TextScaled=true,Font=Enum.Font.Gotham,TextStrokeTransparency=0.2,
    })
...
local FarmAtk={
    ...
    Kiếm=function(ch,mob)
        ...
        for _,t in pairs(ch:GetChildren()) do
            ...
            if t:IsA("Tool") and (n:find("sword") or n:find("blade") or n:find("kiem")) then
                for _,e in pairs(t:GetDescendants()) do
    ...
    Súng=function(ch,mob)
        ...
        for _,t in pairs(ch:GetChildren()) do
            local n=t.Name:lower()
            if t:IsA("Tool") and (n:find("gun") or n:find("pistol") or n:find("rifle")) then
...
UserInputService.InputBegan:Connect(function(i,gp)
    ...
    if i.KeyCode==Enum.KeyCode.M  then ToggleMenu() end
    if i.KeyCode==Enum.KeyCode.F1 then
        local v=not Toggles.Ghost; Toggles.Ghost=v; togGhost.Set(v)
    end
    if i.KeyCode==Enum.KeyCode.F2 then
        local v=not Toggles.NightVision; Toggles.NightVision=v; togNV.Set(v)
    end
end)

print("⚡ MV X SHINN DEV v5.2 — LOADED")
